...
main() {
  group("Decoding", () {
      ...
      group("MsgPack", () {
        for (var row in DATA) {
          test(row[0].toString(), () {
            expect(DECODER.convert(row[1]).toString(),
      ...
      group("JSON", () {
        for (var row in DATA) {
          test(row[0].toString(), () {
            expect(DECODER.convert(row[2]).toString(),
                equals(_ignoreMapEnforcer(row[0]).toString()));
          });
        }
      });
      ...
      group("verbose-JSON", () {
        for (var row in DATA) {
          test(row[0].toString(), () {
            expect(DECODER.convert(row[3]).toString(),
                equals(_ignoreMapEnforcer(row[0]).toString()));
          });
        }
      });
      ...
      test("recursive-AsMapKey", () {
        expect(
            ...
            equals({"cached": 0, new TransitSymbol("Explain"): "cached"}
                .toString()));
      });
    });
}
